...
namespace hydra::N64 {
    ...
    void hungry_for_more(ma_device* device, void* out, const void* in, ma_uint32 frames) {
        auto& ai = *static_cast<Ai*>(device->pUserData);
        if (ai.ai_frequency_ == 0)
            return;
        // frame: 2 channels * 2 bytes per sample
        auto bytes_per_frame = ma_get_bytes_per_frame(device->playback.format, device->playback.channels);
        ...
        struct Deleter {
            Deleter(ma_resampler& resampler) : resampler(resampler) {};
            ~Deleter() {
        ...
        ma_result result = ma_resampler_process_pcm_frames(&resampler, ai.ai_buffer_.data(), &frames_in, buffer.data(), &frames_out);
    ...
    void Ai::WriteWord(uint32_t addr, uint32_t data) {
        switch (addr) {
            ...
            case AI_DACRATE: {
                uint32_t old_freq = ai_frequency_;
