...
static inline uint16_t rgba32_to_rgba16(uint32_t color) {
    ...
    return __builtin_bswap16((r << 11) | (g << 6) | (b << 1) | 1);
}
...
namespace hydra::N64 {
    ...
    constexpr inline std::string get_rdp_command_name(RDPCommandType type) {
        switch (type) {
            #define X(name, opcode, length) case RDPCommandType::name: return #name;
            RDP_COMMANDS
            #undef X
    ...
    constexpr inline int get_rdp_command_length(RDPCommandType type) {
        switch (type) {
            #define X(name, opcodes, length) case RDPCommandType::name: return length;
            RDP_COMMANDS
            #undef X
            default: Logger::Warn("Unknown command length for command: {} ({:02x})", get_rdp_command_name(type), static_cast<int>(type)); return 1;
        }
    }
    ...
    uint32_t RDP::ReadWord(uint32_t addr) {
        switch (addr) {
            case DP_START: return start_address_;
            case DP_END: return end_address_;
            case DP_STATUS: return status_.full;
            case DP_CLOCK: return 0; // ???
            case DP_BUSY: return 0; // ???
            default: {
    ...
    void RDP::WriteWord(uint32_t addr, uint32_t value) {
        switch (addr) {
            ...
            case DP_STATUS: {
                ...
                write.full = value;
                #define flag(x) if (write.clear_##x && !write.set_##x) status_.x = 0; else if (write.set_##x && !write.clear_##x) status_.x = 1;
    ...
    void RDP::process_commands() {
        ...
        status_.freeze = 1;
        while (current < end) {
            uintptr_t address = status_.dma_source_dmem ? reinterpret_cast<uintptr_t>(spmem_ptr_) : reinterpret_cast<uintptr_t>(rdram_ptr_);
            uint64_t data = __builtin_bswap64(*reinterpret_cast<uint64_t*>(address + current));
            ...
            if (command_type >= 8) {
                ...
                for (int i = 0; i < length; i++) {
                    command[i] = __builtin_bswap64(*reinterpret_cast<uint64_t*>(address + current + (i * 8)));
                }
                execute_command(command);
                // Logger::Info("RDP: Command {} ({:02x})", get_rdp_command_name(static_cast<RDPCommandType>(command_type)), static_cast<int>(command_type));
                current += length * 8;
            } else {
                current += 8;
            }
    ...
    void RDP::execute_command(const std::vector<uint64_t>& data) {
        RDPCommandType command = static_cast<RDPCommandType>((data[0] >> 56) & 0b111111);
        ...
        switch (command) {
            ...
            case RDPCommandType::SetFillColor: {
                ...
                fill_color_ = __builtin_bswap32(fill_color_);
                break;
            }
            case RDPCommandType::LoadTile: {
                ...
                for (int t = tl; t < th; t++) {
                    for (int s = sl; s < sh; s++) {
                        uint16_t src = *reinterpret_cast<uint16_t*>(&rdram_ptr_[texture_dram_address_latch_ + (t * texture_width_latch_ + s) * 2]);
                        uint16_t* dst = reinterpret_cast<uint16_t*>(&tmem_[tile.tmem_address + (t - tl) * tile.line_width + (s - sl) * 2]);
                        *dst = src;
                    }
                }
                break;
            ...
            case RDPCommandType::LoadBlock: {
                ...
                for (int i = sl; i < sh; i += 8) {
                    uint64_t src = *reinterpret_cast<uint64_t*>(&rdram_ptr_[texture_dram_address_latch_ + i]);
                    if (odd) {
                        src = (src >> 32) | (src << 32);
                    }
            ...
            case RDPCommandType::SetEnvironmentColor: {
                ...
                environment_color_ = __builtin_bswap32(environment_color_);
                break;
            }
            case RDPCommandType::SetBlendColor: {
                blend_color_ = data[0] & 0xFFFFFFFF;
                blend_color_ = __builtin_bswap32(blend_color_);
                break;
            }
            case RDPCommandType::SetPrimitiveColor: {
                primitive_color_ = data[0] & 0xFFFFFFFF;
                primitive_color_ = __builtin_bswap32(primitive_color_);
                break;
            }
            ...
            case RDPCommandType::SetFogColor: {
                ...
                fog_color_ = __builtin_bswap32(fog_color_);
                break;
            }
            case RDPCommandType::SetCombineMode: {
                ...
                switch (command.sub_A_RGB_1) {
                    case 0: color_sub_a_ = &combined_color_; break;
                    case 1: color_sub_a_ = &texel_color_0_; break;
                    case 2: color_sub_a_ = &texel_color_1_; break;
                    case 3: color_sub_a_ = &primitive_color_; break;
                    case 4: color_sub_a_ = &shade_color_; break;
                    case 5: color_sub_a_ = &environment_color_; break;
                    case 6: color_sub_a_ = &color_one_; break;
                    // TODO: Implement noise
                    case 7: color_sub_a_ = &color_zero_; break;
                    case 8 ... 15: color_sub_a_ = &color_zero_; break;
                }

                switch (command.sub_B_RGB_1) {
                    case 0: color_sub_b_ = &combined_color_; break;
                    case 1: color_sub_b_ = &texel_color_0_; break;
                    case 2: color_sub_b_ = &texel_color_1_; break;
                    case 3: color_sub_b_ = &primitive_color_; break;
                    case 4: color_sub_b_ = &shade_color_; break;
                    case 5: color_sub_b_ = &environment_color_; break;
                    // TODO: Key center??
                    case 6: color_sub_b_ = &color_zero_; break;
                    // TODO: Convert K4??
                    case 7: color_sub_b_ = &color_zero_; break;
                    case 8 ... 15: color_sub_b_ = &color_zero_; break;
                }

                switch (command.mul_RGB_1) {
                    case 0: color_multiplier_ = &combined_color_; break;
                    case 1: color_multiplier_ = &texel_color_0_; break;
                    case 2: color_multiplier_ = &texel_color_1_; break;
                    case 3: color_multiplier_ = &primitive_color_; break;
                    case 4: color_multiplier_ = &shade_color_; break;
                    case 5: color_multiplier_ = &environment_color_; break;
                    // TODO: rest of the colors
                    case 16 ... 31: color_multiplier_ = &color_zero_; break;
                    default: std::cout << "Bad multiplier: " << command.mul_RGB_1 << std::endl; break;
                }

                switch (command.add_RGB_1) {
                    case 0: color_adder_ = &combined_color_; break;
                    case 1: color_adder_ = &texel_color_0_; break;
                    case 2: color_adder_ = &texel_color_1_; break;
                    case 3: color_adder_ = &primitive_color_; break;
                    case 4: color_adder_ = &shade_color_; break;
                    case 5: color_adder_ = &environment_color_; break;
                    case 6: color_adder_ = &color_one_; break;
                    case 7: color_adder_ = &color_zero_; break;
                }
                break;
            }
            default: Logger::Debug("Unhandled command: {} ({:02x})", get_rdp_command_name(command), static_cast<int>(command)); break;
        }
    }

    void RDP::draw_pixel(int x, int y) {
        uintptr_t address = reinterpret_cast<uintptr_t>(rdram_ptr_) + framebuffer_dram_address_ + (y * framebuffer_width_ + x) * (framebuffer_pixel_size_ >> 3);
        switch (cycle_type_) {
    ...
    void RDP::color_combiner() {
        uint8_t r = ((((*color_sub_a_ & 0xFF) - (*color_sub_b_ & 0xFF)) * (*color_multiplier_ & 0xFF)) / 0xFF) + (*color_adder_ & 0xFF);
        uint8_t g = (((((*color_sub_a_ >> 8) & 0xFF) - ((*color_sub_b_ >> 8) & 0xFF)) * ((*color_multiplier_ >> 8) & 0xFF)) / 0xFF) + ((*color_adder_ >> 8) & 0xFF);
        uint8_t b = (((((*color_sub_a_ >> 16) & 0xFF) - ((*color_sub_b_ >> 16) & 0xFF)) * ((*color_multiplier_ >> 16) & 0xFF)) / 0xFF) + ((*color_adder_ >> 16) & 0xFF);
        uint8_t a = 0xFF;
    ...
    uint32_t RDP::blender() {
        ...
        switch (blender_1a_0_) {
            case 0: color1 = combined_color_; break;
            case 1: color1 = framebuffer_color_; break;
            case 2: color1 = blend_color_; break;
            case 3: color1 = fog_color_; break;
        }

        switch (blender_2a_0_) {
            case 0: color2 = combined_color_; break;
            case 1: color2 = framebuffer_color_; break;
            case 2: color2 = blend_color_; break;
            case 3: color2 = fog_color_; break;
        }

        switch (blender_1b_0_) {
            case 0: multiplier1 = combined_color_ >> 24; break;
            case 1: multiplier1 = fog_color_ >> 24; break;
            case 2: multiplier1 = shade_color_ >> 24; break;
            case 3: multiplier1 = 0x00; break;
        }

        switch (blender_2b_0_) {
            case 0: multiplier2 = ~multiplier1; break;
            case 1: multiplier2 = framebuffer_color_ >> 24; break;
            case 2: multiplier2 = 0xFF; break;
            case 3: multiplier2 = 0x00; break;
        }

        float r = (((color1 & 0xFF) * multiplier1) + ((color2 & 0xFF) * multiplier2)) / (multiplier1 + multiplier2);
        float g = ((((color1 >> 8) & 0xFF) * multiplier1) + (((color2 >> 8) & 0xFF) * multiplier2)) / (multiplier1 + multiplier2);
        float b = ((((color1 >> 16) & 0xFF) * multiplier1) + (((color2 >> 16) & 0xFF) * multiplier2)) / (multiplier1 + multiplier2);
    ...
    uint32_t RDP::z_get(int x, int y) {
        uintptr_t address = reinterpret_cast<uintptr_t>(rdram_ptr_) + zbuffer_dram_address_ + (y * framebuffer_width_ + x) * 2;
        uint16_t* ptr = reinterpret_cast<uint16_t*>(address);
    ...
    constexpr std::array<uint8_t, 8> z_shifts = {
        6, 5, 4, 3, 2, 1, 0, 0
    };
    ...
    void RDP::fetch_texels(int tile, int32_t s, int32_t t, int32_t w) {
    ...
    void RDP::edgewalker(const std::vector<uint64_t>& data) {
        ...
        if constexpr (Shade) {
            r = (((data[next_block] >> 48) & 0xFFFF) << 16) | ((data[next_block + 2] >> 48) & 0xFFFF);
            g = (((data[next_block] >> 32) & 0xFFFF) << 16) | ((data[next_block + 2] >> 32) & 0xFFFF);
            b = (((data[next_block] >> 16) & 0xFFFF) << 16) | ((data[next_block + 2] >> 16) & 0xFFFF);
            a = (((data[next_block] >> 0) & 0xFFFF) << 16) | ((data[next_block + 2] >> 0) & 0xFFFF);

            DrDx = (((data[next_block + 1] >> 48) & 0xFFFF) << 16) | ((data[next_block + 3] >> 48) & 0xFFFF);
            DgDx = (((data[next_block + 1] >> 32) & 0xFFFF) << 16) | ((data[next_block + 3] >> 32) & 0xFFFF);
            DbDx = (((data[next_block + 1] >> 16) & 0xFFFF) << 16) | ((data[next_block + 3] >> 16) & 0xFFFF);
            DaDx = (((data[next_block + 1] >> 0) & 0xFFFF) << 16) | ((data[next_block + 3] >> 0) & 0xFFFF);

            DrDe = (((data[next_block + 4] >> 48) & 0xFFFF) << 16) | ((data[next_block + 6] >> 48) & 0xFFFF);
            DgDe = (((data[next_block + 4] >> 32) & 0xFFFF) << 16) | ((data[next_block + 6] >> 32) & 0xFFFF);
            DbDe = (((data[next_block + 4] >> 16) & 0xFFFF) << 16) | ((data[next_block + 6] >> 16) & 0xFFFF);
            DaDe = (((data[next_block + 4] >> 0) & 0xFFFF) << 16) | ((data[next_block + 6] >> 0) & 0xFFFF);

            DrDy = (((data[next_block + 5] >> 48) & 0xFFFF) << 16) | ((data[next_block + 7] >> 48) & 0xFFFF);
            DgDy = (((data[next_block + 5] >> 32) & 0xFFFF) << 16) | ((data[next_block + 7] >> 32) & 0xFFFF);
            DbDy = (((data[next_block + 5] >> 16) & 0xFFFF) << 16) | ((data[next_block + 7] >> 16) & 0xFFFF);
            DaDy = (((data[next_block + 5] >> 0) & 0xFFFF) << 16) | ((data[next_block + 7] >> 0) & 0xFFFF);
        ...
        if constexpr (Texture) {
            s = (((data[next_block] >> 48) & 0xFFFF) << 16) | ((data[next_block + 2] >> 48) & 0xFFFF);
            t = (((data[next_block] >> 32) & 0xFFFF) << 16) | ((data[next_block + 2] >> 32) & 0xFFFF);
            w = (((data[next_block] >> 16) & 0xFFFF) << 16) | ((data[next_block + 2] >> 16) & 0xFFFF);

            DsDx = (((data[next_block + 1] >> 48) & 0xFFFF) << 16) | ((data[next_block + 3] >> 48) & 0xFFFF);
            DtDx = (((data[next_block + 1] >> 32) & 0xFFFF) << 16) | ((data[next_block + 3] >> 32) & 0xFFFF);
            DwDx = (((data[next_block + 1] >> 16) & 0xFFFF) << 16) | ((data[next_block + 3] >> 16) & 0xFFFF);

            DsDe = (((data[next_block + 4] >> 48) & 0xFFFF) << 16) | ((data[next_block + 6] >> 48) & 0xFFFF);
            DtDe = (((data[next_block + 4] >> 32) & 0xFFFF) << 16) | ((data[next_block + 6] >> 32) & 0xFFFF);
            DwDe = (((data[next_block + 4] >> 16) & 0xFFFF) << 16) | ((data[next_block + 6] >> 16) & 0xFFFF);

            DsDy = (((data[next_block + 5] >> 48) & 0xFFFF) << 16) | ((data[next_block + 7] >> 48) & 0xFFFF);
            DtDy = (((data[next_block + 5] >> 32) & 0xFFFF) << 16) | ((data[next_block + 7] >> 32) & 0xFFFF);
            DwDy = (((data[next_block + 5] >> 16) & 0xFFFF) << 16) | ((data[next_block + 7] >> 16) & 0xFFFF);
                ...
                if (x >= scissor_xh_ && x < scissor_xl_ && y >= scissor_yh_ && y < scissor_yl_) {
                    ...
                    if (pass_depth_test) {
                        if constexpr (Texture) {
                            ...
                            fetch_texels(tile, s_line, t_line, w_line);
                        }
                        draw_pixel(x, y);
                    }
