...
namespace hydra::NES {
    ...
    bool CPUBus::LoadCartridge(std::string path) {
        ...
        if (ifs.is_open()) {
            ...
            mapper_ = (header_.flags_6 >> 4) | (header_.flags_7 & 0xF0) | ((header_.mapper_msb & 0xF) << 8);
            refill_prg_map();
            ram_.fill(0);
        } else {
            return false;
    ...
    uint8_t CPUBus::redirect_address_r(uint16_t addr) {
        if (addr >= 0x2000 && addr <= 0x3FFF) {
            ...
            switch (addr) {
                case 0x4016: return joypad1_.ReadCurrent();
                case 0x4017: return joypad2_.ReadCurrent();
            }
        }
        return last_read_;
    }
    ...
    void CPUBus::refill_prg_map() {
        ...
        switch (mapper_) {
            ...
            default: Logger::Fatal("Mapper {} not implemented", mapper_);
        }
    }
