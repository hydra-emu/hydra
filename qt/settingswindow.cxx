...
#define add_item(var_name, name, image)                                            \
    ...
    tab_list_->addItem(var_name)
        ...
        {
            std::filesystem::path path = Settings::GetCachePath() /
                                         std::string(Settings::GetCoreInfo()[i].core_name + ".png");
            if (std::filesystem::exists(path))
            {
                QListWidgetItem* item = new QListWidgetItem(
                    QPixmap((path).string().c_str()), Settings::GetCoreInfo()[i].core_name.c_str());
                tab_list_->addItem(item);
            }
            else
            {
                auto wrapper = hydra::EmulatorFactory::Create(Settings::GetCoreInfo()[i].path);
                ...
                if (wrapper->GetInfo(hydra::InfoType::IconData))
                {
                    ...
                    if (width <= 0 || height <= 0)
                    {
                        printf("Invalid icon size in core %s\n",
                               Settings::GetCoreInfo()[i].core_name.c_str());
                        continue;
                    }
                    ...
                    QListWidgetItem* item = new QListWidgetItem(
                        QPixmap::fromImage(image), Settings::GetCoreInfo()[i].core_name.c_str());
                    tab_list_->addItem(item);
                }
                else
                {
                    add_item(core, Settings::GetCoreInfo()[i].core_name.c_str(), "core.png");
...
void SettingsWindow::create_tabs()
{
    ...
    }
    {
        ...
        for (size_t i = 0; i < Settings::GetCoreInfo().size(); i++)
        {
            const auto& core = Settings::GetCoreInfo()[i];
        ...
        connect(core_list, &QListWidget::itemDoubleClicked, this,
                [html, core_list](QListWidgetItem* item) {
                    ...
                    auto core = Settings::GetCoreInfo()[index];
                    QMessageBox msg;
                    msg.setWindowTitle(core.core_name.c_str());
                    std::filesystem::path path =
                        Settings::GetCachePath() /
                        std::string(Settings::GetCoreInfo()[index].core_name + ".png");
                    QPixmap pixmap;
                    if (!std::filesystem::exists(path))
                    {
