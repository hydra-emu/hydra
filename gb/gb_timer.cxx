...
namespace hydra::Gameboy
{
    Timer::Timer(ChannelArrayPtr channel_array_ptr, Bus& bus)
        : channel_array_ptr_(channel_array_ptr), bus_(bus), DIV(bus.GetReference(addr_div)),
          TIMA(bus.GetReference(addr_tim)), TAC(bus.GetReference(addr_tac)),
          TMA(bus.GetReference(addr_tma)), IF(bus.GetReference(addr_ifl))
    {
    }
    ...
    bool Timer::Update(uint8_t cycles, uint8_t old_if)
    {
        ...
        if (tima_overflow_)
        {
            ...
            if (TIMA == 0)
            {
                ...
                TIMA = TMA;
                // If this isn't true, IF has changed during this instruction so the new value
                // persists
                if (IF == old_if)
                {
