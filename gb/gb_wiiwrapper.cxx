...
{
    if ((key & WPAD_BUTTON_RIGHT) || (key & WPAD_BUTTON_LEFT) || (key & WPAD_BUTTON_UP) ||
        (key & WPAD_BUTTON_DOWN))
    {
        auto index = (key & WPAD_BUTTON_DOWN)    ? 0
                     : (key & WPAD_BUTTON_UP)    ? 1
                     : (key & WPAD_BUTTON_RIGHT) ? 2
                     : (key & WPAD_BUTTON_LEFT)  ? 3
                                                 : 0;
        bus_.DirectionKeys &= (~(1UL << index));
        interrupt_flag_ |= IFInterrupt::JOYPAD;
    }
    if ((key & WPAD_BUTTON_1) || (key & WPAD_BUTTON_2) || (key & WPAD_BUTTON_PLUS) ||
        (key & WPAD_BUTTON_MINUS))
    {
        auto index = (key & WPAD_BUTTON_1)       ? 0
                     : (key & WPAD_BUTTON_2)     ? 1
                     : (key & WPAD_BUTTON_MINUS) ? 2
                     : (key & WPAD_BUTTON_PLUS)  ? 3
                                                 : 0;
