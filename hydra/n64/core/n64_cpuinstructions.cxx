...
namespace hydra::N64 {
    ...
    uint32_t CPU::get_cp0_register_32(uint8_t reg) {
        switch (reg) {
            case CP0_INDEX: return cp0_regs_[reg].UW._0 & 0x8000'003F;
            case CP0_COUNT: return cpubus_.time_ >> 1;
            case CP0_CAUSE: {
            ...
            case 7:
            case 21 ... 25:
            case 31: return cp0_weirdness_;
            default: return cp0_regs_[reg].UW._0;
        }
    }
    ...
    uint64_t CPU::get_cp0_register_64(uint8_t reg) {
        switch (reg) {
            ...
            default: Logger::Fatal("Reading 64 bits from COP0 register {}", reg); return 0;
        }
    }
    ...
    void CPU::set_cp0_register_32(uint8_t reg, uint32_t val) {
        ...
        switch (reg) {
            ...
            case CP0_XCONTEXT: {
                CP0XContext.full = (value & 0xFFFFFFFE00000000) | (CP0XContext.full & 0x00000001FFFFFFFF);
                break;
            }
            case CP0_ENTRYHI: {
    ...
    void CPU::set_cp0_register_64(uint8_t reg, uint64_t value) {
        switch (reg) {
            ...
            case CP0_XCONTEXT: {
                cp0_regs_[reg].UD = (value & 0xFFFFFFFE00000000) | (cp0_regs_[reg].UD & 0x00000001FFFFFFFF);
                break;
            }
            case CP0_LLADDR:
                value &= 0xFFFF'FFFF;
