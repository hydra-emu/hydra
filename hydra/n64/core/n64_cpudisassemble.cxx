...
namespace hydra::N64 {
    std::string disassemble_instr_special(uint32_t instr_num, bool register_names) {
        ...
        switch (static_cast<InstructionType>(instr.RType.func + 64)) {
            ...
            case InstructionType::s_SLL: {
                ...
                ret += fmt::format("{}, {}, {}", gpr_get_name(instr.RType.rd, register_names), gpr_get_name(instr.RType.rt, register_names), static_cast<uint8_t>(instr.RType.sa));
                break;
            }
            ...
            case InstructionType::s_ADDU: {
                ret += fmt::format("{}, {}, {}", gpr_get_name(instr.RType.rd, register_names), gpr_get_name(instr.RType.rs, register_names), gpr_get_name(instr.RType.rt, register_names));
                break;
            }
        }
        return ret;
    ...
    std::string disassemble_instr(uint32_t instr_num, bool register_names) {
        ...
        switch (static_cast<InstructionType>(instr.IType.op)) {
            ...
            case InstructionType::ANDI: {
                ret += fmt::format("{}, {}, 0x{:04x}", gpr_get_name(instr.IType.rt, register_names), gpr_get_name(instr.IType.rs, register_names), static_cast<int16_t>(instr.IType.immediate));
                break;
            }
            case InstructionType::BEQ:
            case InstructionType::BNE: {
                ret += fmt::format("{}, {}, 0x{:04x}", gpr_get_name(instr.IType.rs, register_names), gpr_get_name(instr.IType.rt, register_names), static_cast<uint16_t>(instr.IType.immediate));
                break;
            }
            case InstructionType::BGTZ:
            case InstructionType::BLEZ: {
                ret += fmt::format("{}, 0x{:04x}", gpr_get_name(instr.IType.rs, register_names), static_cast<uint16_t>(instr.IType.immediate));
                break;
            }
            case InstructionType::LUI: {
                ret += fmt::format("{}, 0x{:04x}", gpr_get_name(instr.IType.rt, register_names), static_cast<uint16_t>(instr.IType.immediate));
                break;
            }
            ...
            case InstructionType::LBU: {
                if (instr.IType.immediate == 0) {
                    ret += fmt::format("{}, ({})", gpr_get_name(instr.IType.rt, register_names), gpr_get_name(instr.IType.rs, register_names));
                } else {
                    ret += fmt::format("{}, 0x{:04x}({})", gpr_get_name(instr.IType.rt, register_names), static_cast<uint16_t>(instr.IType.immediate), gpr_get_name(instr.IType.rs, register_names));
                }
                break;
            }
    ...
    std::vector<DisassemblerInstruction> CPU::disassemble(uint64_t start_vaddr, uint64_t end_vaddr, bool register_names) {
        ...
        for (uint64_t i = start_vaddr; i < end_vaddr; i += 4) {
            uint32_t instr = __builtin_bswap32(*reinterpret_cast<uint32_t*>(cpubus_.redirect_paddress(translate_vaddr(i).paddr)));
            std::string instr_str = disassemble_instr(instr, register_names);
            DisassemblerInstruction instr_s = { .vaddr = static_cast<uint32_t>(i), .disassembly = instr_str };
            ret[j++] = instr_s;
        }
        return ret;
    }
