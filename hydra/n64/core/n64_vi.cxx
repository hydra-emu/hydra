...
namespace hydra::N64 {
    ...
    bool Vi::Redraw() {
        ...
        width_ >>= 10;
        height_ >>= 10;
        auto new_size = width_ * height_ * 4;
        ...
        switch (pixel_mode_) {
            case 0b11: {
                for (int y = 0; y < height_; y++) {
                    for (int x = 0; x < width_; x++) {
                        uint32_t color = (reinterpret_cast<uint32_t*>(memory_ptr_))[(y * vi_width_) + x];
                        set_pixel(x, y, color);
                    }
                }
                break;
            ...
            case 0b10: {
                for (int y = 0; y < height_; y++) {
                    for (int x = 0; x < width_; x++) {
                        uint16_t color_temp = (reinterpret_cast<uint16_t*>(memory_ptr_))[(y * vi_width_) + x];
