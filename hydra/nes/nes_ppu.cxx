...
namespace hydra::NES {
    ...
    void PPU::fetch_sprite_data() {
        ...
        switch (current_cycle & 0b111) {
            case 5: {
                sprite_shift_registers_[current_sprite].first = read(sprite_pattern_address_ + secondary_oam_[current_sprite].tile_index * 16 + (scanline_ - secondary_oam_[current_sprite].y));
                if (flip_x)
                    sprite_shift_registers_[current_sprite].first = reverse(sprite_shift_registers_[current_sprite].first);
                break;
            }
            case 7: {
                sprite_shift_registers_[current_sprite].second = read(sprite_pattern_address_ + secondary_oam_[current_sprite].tile_index * 16 + (scanline_ - secondary_oam_[current_sprite].y) + 8);
                if (flip_x)
                    sprite_shift_registers_[current_sprite].second = reverse(sprite_shift_registers_[current_sprite].second);
                break;
            }
            case 3: {
    ...
    void PPU::draw_pixel() {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (sprite_active_[i]) {
                uint8_t sprite_cur = ((sprite_shift_registers_[i].second >> 7) << 1) | (sprite_shift_registers_[i].first >> 7);
